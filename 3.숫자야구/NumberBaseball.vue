<template>
    <div>
        ...
        <form @submit.prevent="onSubmitForm">
            <!--v-on:submit축약형 + event.preventDefault()-->
...
<script>
    ...
    export default {
        ...
        methods: {
            onSubmitForm() {
                if (this.value === this.answer.join('')){ //정답 맞췄으면 value(문자), answer[배열] 이기때문에 조인..
                    this.tries.push({
                        try: this.value,
                        result: '홈런',
                    });
                    ...
                    if(this.tries.length >= 9 ){//10번시도했을 때
                    ...
                    const answerArray = this.value.split('').map(v => parseInt(v)); //value 문자열을 숫자배열로 바꾸는..
                    for(let i = 0; i <4; i+=1){
